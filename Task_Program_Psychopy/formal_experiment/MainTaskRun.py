...
dataFile = open("%s.csv" % (info['num']+'_'+info['name']+'_'+info['RUN']+'_scan'), 'a')
dataFile.write(info['num']+','+info['name']+','+info['RUN']+'\n')
dataFile.write(u' 导航编号, 界面, 标准时间, 界面开始, 界面结束, 持续时间, 起点, 终点, 最佳步数, U-turn-cost,'
               u' 按键次数, 当前站台, 当前站台类型, 最优剩余步数, 最优还要经过几个换乘站, 最优还要换乘次数, 最优还要转弯次数, 最佳U,'
               u' 反应时, 选择方向, 被试按键所走线路, '
               u' 到达站点, 到达站台类型, 已经经过的换乘站数, 已经换乘的次数, 已经转弯次数, 反应类型\n')
...
win.flip()
core.wait(0)
k_syn = event.waitKeys(keyList='s')     # 等待s信号进行同步
...
# trial
        ...
        step_truth = 0
# Du是路线确定后就确定好的
        Du = step - (abs(station_2[0] - station_0[0]) + abs(station_2[1] - station_0[1]))
...
# 这里计算的都是按键前站的参数
# 当前站
            station_cur = station_1
# 当前站的类型
            station_type = judge_function.get_station_type(station_1)
# 计算理论上最优还有几步到达：
            need_step = optimal_math.get_need_steps(station_1, station_2)
# 计算理论上最优还有几个换成站
            optimal_ex_station = len(list(set(ex_stations) & set(optimal_math.get_idea_trail(station_1, station_2))))
            if station_1 in ex_stations:
                optimal_ex_station -= 1
# 计算理论上最优路还要换乘几次
            optimal_ex_time = optimal_math.get_idea_line_number(station_1, station_2) - 1
# 计算最佳u
            optimal_Du = need_step - (abs(station_2[0] - station_1[0]) + abs(station_2[1] - station_1[1]))
# 计算最优还要转弯的次数
            optimal_turn_time = parameter_function.get_turn_time(optimal_math.get_idea_trail(station_1, station_2))
...
# 按键后的参数
# 已经经过的换乘站数
                ex_sta_num = parameter_function.get_ex_sta_num(station_1, trail_truth)
# 已经换乘的次数
                ex_line_time = parameter_function.get_ex_line_time(trail_truth)
# 计算路途中转弯的次数
                turn_time = parameter_function.get_turn_time(trail_truth)
...
# 写入表格
                dataFile.write(str(journey_time) + ', ' + u'navigation' + ', ' + str(date)
                               ...
                               + str(Du) + ', '
                               + str(trial_time) + ', ' + station_list_str[station_list.index(station_cur)] + ', '
                               + str(station_type) + ', ' + str(need_step) + ', ' + str(optimal_ex_station) + ', '
                               + str(optimal_ex_time) + ', ' + str(optimal_turn_time) + ', ' + str(optimal_Du) + ', '
                               + 'N/A' + ', ' + 'no_response' + ', ' + 'no_response' + ', '
                               + station_list_str[station_list.index(station_cur)] + ', '
                               + str(station_type) + ',' + str(ex_sta_num) + ','
                               + str(ex_line_time) + ', ' + str(turn_time) + ', ' + 'none' + ', ' + '\n')
# 先呈现黑屏
                black_page(1, 3)
                dataFile.write(str(journey_time) + ', ' + u'interval' + ', ' + str(date) + ', ' + str(screen_begin)
                               + ', ' + str(screen_end) + ', ' + str(lasting) + '\n')
...
# 这里计算被试到达站的参数
# 被试到达站的类型
                station_type01 = judge_function.get_station_type(station_1)
# 被试所走的实际路线
                trail_truth.append(station_1)
# 在这里写入反应类型的算法语句
                response_type01 = parameter_function.get_response_type(trail_truth)
# 被试在这里所走的线路
                line_cur = judge_function.which_line2(station_1, trail_truth[-2])
# 计算路途中的换线次数ex_line_time
                ex_line_time = parameter_function.get_ex_line_time(trail_truth)
# 计算路途中换乘站的数量ex_sta_num
                ex_sta_num = parameter_function.get_ex_sta_num(station_1, trail_truth)
# 计算路途中转弯的次数
                turn_time = parameter_function.get_turn_time(trail_truth)
...
# 写入表格
                dataFile.write(str(journey_time) + ', ' + u'navigation' + ', ' + str(date)
                               ...
                               + str(station_type) + ', ' + str(need_step) + ', ' + str(optimal_ex_station) + ', '
                               + str(optimal_ex_time) + ', ' + str(optimal_turn_time) + ', ' + str(optimal_Du) + ', '
                               + str(timeUse) + ', ' + k_1[0] + ', ' + str(line_cur) + ', '
                               + station_list_str[station_list.index(station_1)] + ', '
                               + str(station_type01) + ',' + str(ex_sta_num) + ','
                               + str(ex_line_time) + ', ' + str(turn_time) + ', ' + str(response_type01) + ', ' + '\n')
# 每次按键后的黑屏1~3秒
            ...
            # 成功到达（额外记录奖赏信息）
            success_page()
            dataFile.write(str(journey_time) + ', ' + u'trip-end' + ', ' + str(date) + ', ' + str(screen_begin) + ', '
                           + str(screen_end) + ', ' + str(lasting) + ', ' + str(reward) + '\n')
# 完成journey后的黑屏2~3秒
        black_page(2, 3)
        dataFile.write(str(journey_time) + ', ' + u'interval' + ', ' + str(date) + ', ' + str(screen_begin) + ', '
                       + str(screen_end) + ', ' + str(lasting) + '\n')
