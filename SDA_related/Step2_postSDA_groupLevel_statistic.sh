...
inputImg=${wkDir}/MVPA_wholebrain_lineDs_design1
outDir=${wkDir}/postMVPA_searchlight_wholebrain_lineDs_accuracy_design2
brainMask=${wkDir}/gre_mask_flirt_bin.nii.gz
matchPattern=*accuracy.nii.gz
subChanceLevel=yes
chanceLevel=0.25
smoothImg=yes
smoothFWHM=6

# ensuring the output directory
if [[ -e ${outDir} ]]; then
	echo "File exsisted!"
	exit 0
else
...
if [[ ${smoothImg} == "yes" ]]; then
	...
	for fileInd in `ls ${inputImg}/${matchPattern}`; do
		fileList="${fileList} ${fileInd}"
	done
fi
fslmerge -t ${outDir}/Merge_data.nii.gz ${fileList}
if [[ ${subChanceLevel} == 'yes' ]]; then
	fslmaths ${outDir}/Merge_data.nii.gz -sub ${chanceLevel} ${outDir}/Merge_data.nii.gz
fi
